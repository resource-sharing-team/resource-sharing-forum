...
const mockCatalog: AdminCatalogData = {
  categories: clone(categories.filter((item) => item.type !== "标签")).map(
    (item) => ({ ...item, kind: "CATEGORY" }),
  ),
  tags: clone(categories.filter((item) => item.type === "标签")).map(
    (item) => ({ ...item, kind: "TAG" }),
  ),
  rows: clone(categories).map((item) => ({
    ...item,
    kind: item.type === "标签" ? "TAG" : "CATEGORY",
  })),
};
const mockConfig: AdminConfigData = {
  memberLevels: clone(memberLevels).map((item, index) => ({
    ...item,
    rawId: index + 1,
    id: String(index + 1),
  })),
  scoreRules: [
    {
      key: "point.resource_approved",
      label: "资源审核通过 + 积分",
      value: "10",
      valueType: "INTEGER",
    },
    {
      key: "point.resource_downloaded",
      label: "资源被下载 + 积分/次",
      value: "2",
      valueType: "INTEGER",
    },
    {
      key: "point.request_accepted",
      label: "求资源回答被采纳 + 奖励积分",
      value: "5",
      valueType: "INTEGER",
    },
    {
      key: "point.resource_offline_penalty",
      label: "资源违规下架 - 扣积分",
      value: "20",
      valueType: "INTEGER",
    },
    {
      key: "point.comment_delete_penalty",
      label: "评论违规删除 - 扣积分",
      value: "5",
      valueType: "INTEGER",
    },
  ...
  systemParams: [
    {
      ...
      label: "允许上传文件类型",
      value: "pdf,doc,docx,ppt,pptx,xls,xlsx,zip,rar,7z,png,jpg,jpeg,txt,md",
    },
    {
      key: "resource.daily_publish_limit",
      label: "用户每日最大发布资源数",
      value: "5",
      valueType: "INTEGER",
    },
    {
      key: "auth.email_code_minutes",
      label: "邮箱验证码有效期（分钟）",
      value: "10",
      valueType: "INTEGER",
    },
    {
      key: "auth.login_fail_lock_count",
      label: "连续登录失败锁定次数",
      value: "5",
      valueType: "INTEGER",
    },
    {
      key: "auth.login_fail_lock_minutes",
      label: "登录失败锁定时长（分钟）",
      value: "10",
      valueType: "INTEGER",
    },
...
export async function loginAdmin(
  ...
  return {
    ...
    user: {
      ...
      nickname: String(
        payload.user?.nickname || payload.user?.username || "管理员",
      ),
...
export async function getAdminContent(): Promise<AdminContentData> {
  ...
  const response = await apiClient.get<AdminContentData>("/admin/content", {
    params: { size: 100 },
  });
...
): Promise<PageResult<AdminResource | AdminRequestPost | AdminComment>> {
  const pageParams = withDefaultSize(params, 10);
  if (ENABLE_MOCKS) {
    return pageOf(contentSectionRows(section, mockContent), pageParams);
  }
  const response = await apiClient.get<
    | SectionPage<AdminResource | AdminRequestPost | AdminComment>
    | AdminContentData
  >("/admin/content", {
    params: { ...pageParams, section },
  });
  if (
    isPageLike<AdminResource | AdminRequestPost | AdminComment>(response.data)
  ) {
    return normalizePage(response.data, pageParams);
  }
  return pageOf(
    contentSectionRows(section, withContentDefaults(response.data)),
    pageParams,
  );
}
  ...
  if (ENABLE_MOCKS) {
    ...
    const filtered = mockUsers.filter((item) => {
      const matchesKeyword =
        !keyword ||
        [item.id, item.nickname, item.username, item.email].some((value) =>
          String(value || "").includes(keyword),
        );
      const matchesStatus =
        !status || item.status === status || item.rawStatus === status;
      return matchesKeyword && matchesStatus;
    });
    return pageOf(filtered, params);
  }
  const response = await apiClient.get<PageResult<AdminUser>>("/admin/users", {
    params,
  });
...
export async function getAdminCompliance(): Promise<AdminComplianceData> {
  ...
  const response = await apiClient.get<AdminComplianceData>(
    "/admin/compliance",
    { params: { size: 100 } },
  );
  return {
...
export async function getAdminCompliancePage(
  section: AdminComplianceSection,
  params: Record<string, string> = {},
): Promise<PageResult<AdminReport | AdminComplaint | AdminAppeal>> {
  const pageParams = withDefaultSize(params, 10);
  if (ENABLE_MOCKS) {
    return pageOf(complianceSectionRows(section, mockCompliance), pageParams);
  }
  const response = await apiClient.get<
    | SectionPage<AdminReport | AdminComplaint | AdminAppeal>
    | AdminComplianceData
  >("/admin/compliance", {
    params: { ...pageParams, section },
  });
  ...
  return pageOf(
    complianceSectionRows(section, withComplianceDefaults(response.data)),
    pageParams,
  );
}
...
export async function getAdminCatalog(): Promise<AdminCatalogData> {
  ...
  const response = await apiClient.get<AdminCatalogData>("/admin/catalog", {
    params: { size: 100 },
  });
  return {
...
export async function getAdminCatalogPage(
  ...
  params: Record<string, string> = {},
): Promise<PageResult<AdminCategory>> {
  const pageParams = withDefaultSize(params, 10);
  if (ENABLE_MOCKS) {
    return pageOf(
      filterCatalogRows(catalogSectionRows(section, mockCatalog), pageParams),
      pageParams,
    );
  }
  const response = await apiClient.get<
    SectionPage<AdminCategory> | AdminCatalogData
  >("/admin/catalog", {
    params: { ...pageParams, section },
  });
  ...
  return pageOf(
    catalogSectionRows(section, withCatalogDefaults(response.data)),
    pageParams,
  );
}
...
export async function getAdminCatalogOptions(): Promise<AdminCatalogOptions> {
  ...
  const response = await apiClient.get<AdminCatalogOptions>(
...
export async function deleteResource(id: RawId, reason: string) {
  if (ENABLE_MOCKS)
    return setRowStatus(mockContent.resourceRows, id, "已删除", "DELETED");
  return apiClient.delete(`/admin/resources/${id}`, { data: { reason } });
}

export async function closeRequest(id: RawId, reason: string) {
  if (ENABLE_MOCKS)
    return setRowStatus(mockContent.requestRows, id, "已关闭", "CLOSED");
...
export async function handleReport(
  ...
  if (ENABLE_MOCKS)
    return setRowStatus(
      [...mockCompliance.reports, ...mockCompliance.complaints],
      id,
      status === "RESOLVED" ? "已处理" : "已驳回",
      status,
    );
  return apiClient.post(`/admin/reports/${id}/handle`, {
    status,
    action,
    reason,
    handleResult: reason,
...
export async function handleAppeal(
  ...
  if (ENABLE_MOCKS)
    return setRowStatus(
      mockCompliance.appeals,
      id,
      status === "APPROVED" ? "已处理" : "已驳回",
      status,
    );
  return apiClient.post(`/admin/appeals/${id}/handle`, {
    status,
    reason,
    handleResult: reason,
  });
...
export async function createCategory(payload: CatalogCreatePayload) {
  if (ENABLE_MOCKS) {
    const parent = payload.parentId
      ? findCatalogCategory(payload.parentId)
      : undefined;
...
export async function updateCategory(
  id: RawId,
  payload: Partial<AdminCategory> & { status?: string },
) {
...
export async function disableCategory(id: RawId) {
  if (ENABLE_MOCKS)
    return updateCatalogRow(id, { status: "禁用", rawStatus: "DISABLED" });
...
export async function mergeTags(sourceTagId: RawId, targetTagId: RawId) {
  if (ENABLE_MOCKS)
    return updateCatalogRow(sourceTagId, {
      status: "禁用",
      rawStatus: "DISABLED",
    });
...
export async function updateMemberLevel(level: MemberLevel) {
  ...
  if (ENABLE_MOCKS) {
    const index = mockConfig.memberLevels.findIndex((item) =>
      sameId(item.rawId || item.id, id),
    );
    if (index >= 0)
      mockConfig.memberLevels[index] = {
        ...mockConfig.memberLevels[index],
        ...level,
      };
    return level;
...
export async function updateConfigItem(item: ConfigItem) {
  ...
  return apiClient.put("/admin/config", {
...
function withComplianceDefaults(
  data: Partial<AdminComplianceData>,
): AdminComplianceData {
  return {
...
function withCatalogOptionsDefaults(
  data: Partial<AdminCatalogOptions>,
): AdminCatalogOptions {
  return {
...
function contentSectionRows(
  section: AdminContentSection,
  data: AdminContentData,
): Array<AdminResource | AdminRequestPost | AdminComment> {
...
function complianceSectionRows(
  section: AdminComplianceSection,
  data: AdminComplianceData,
): Array<AdminReport | AdminComplaint | AdminAppeal> {
...
function filterCatalogRows(
  ...
  return rows.filter((item) => {
    const itemLevel =
      item.level ||
      (item.type === "一级分类" ? 1 : item.type === "二级分类" ? 2 : undefined);
    const matchesKeyword = !keyword || item.name.includes(keyword);
    const matchesStatus =
      !status ||
      item.rawStatus === status ||
      item.status === status ||
      item.status === enabledStatusLabel(status);
...
function setRowStatus<
  T extends { id: string; rawId?: RawId; status: string; rawStatus?: string },
>(rows: T[], id: RawId, status: string, rawStatus: string) {
...
function appendCatalogRow(
  partial: Pick<AdminCategory, "name" | "type" | "kind"> &
    Partial<AdminCategory>,
): AdminCategory {
  const id =
    partial.kind === "TAG"
      ? `T${String(mockCatalog.tags.length + 1).padStart(3, "0")}`
      : `F${String(mockCatalog.categories.length + 1).padStart(3, "0")}`;
...
function findCatalogCategory(id: RawId) {
  return mockCatalog.categories.find((item) =>
    sameId(item.rawId || item.id, id),
  );
}
...
function mockCatalogOptions(): AdminCatalogOptions {
  const firstLevelCategories = mockCatalog.categories
    ...
    .map((item) => ({
      ...
      status:
        item.rawStatus || (item.status === "启用" ? "ENABLED" : "DISABLED"),
    }));
  ...
  const tagCandidates = names.map((item) => {
    ...
    return {
      ...
      status:
        tag?.rawStatus ||
        (tag?.status === "禁用" ? "DISABLED" : tag ? "ENABLED" : "MISSING"),
    };
  });
  return {
    firstLevelCategories,
...
function updateCatalogRow(id: RawId, payload: Partial<AdminCategory>) {
  const row = mockCatalog.rows.find((item) =>
    sameId(item.rawId || item.id, id),
  );
  if (row) {
    Object.assign(row, payload);
    if (row.kind === "TAG") {
      const tag = mockCatalog.tags.find((item) =>
        sameId(item.rawId || item.id, id),
      );
      if (tag) Object.assign(tag, row);
    } else {
      const category = mockCatalog.categories.find((item) =>
        sameId(item.rawId || item.id, id),
      );
...
function matchesLogType(actual: string, type: string) {
  ...
  if (normalized === "资源上下架")
    return actual === "资源下架" || actual === "资源恢复";
  if (normalized === "用户状态变更")
    return actual === "用户禁用" || actual === "用户恢复";
  if (normalized === "评论管理")
    return actual === "评论删除" || actual === "评论恢复";
  if (normalized === "举报投诉处理")
    return actual === "举报处理" || actual === "版权投诉处理";
  if (normalized === "分类标签维护")
    return actual.startsWith("分类") || actual.startsWith("标签");
