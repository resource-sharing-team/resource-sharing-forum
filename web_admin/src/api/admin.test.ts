...
describe("admin api pagination adapter", () => {
  ...
  it("paginates legacy content aggregate responses by requested section", async () => {
    ...
    const result = await getAdminContentPage("resource", {
      page: "1",
      size: "1",
    });
  ...
  it("paginates legacy compliance aggregate responses by requested section", async () => {
    ...
    const result = await getAdminCompliancePage("copyright", {
      page: "1",
      size: "10",
    });
  ...
  it("passes catalog level filters to the real section endpoint", async () => {
    ...
    const { getAdminCatalogPage } = await import("./admin");
    const result = await getAdminCatalogPage("category", {
      page: "1",
      size: "10",
      level: "2",
    });
  ...
  it("uses catalog options and normative tag endpoints", async () => {
    ...
    apiClient.post.mockResolvedValueOnce({
      data: { id: 12, name: "文档资料" },
    });
    apiClient.post.mockResolvedValueOnce({
      data: { createdCount: 1, createdTags: ["文档资料"] },
    });

    const { getAdminCatalogOptions, createTag, backfillNormativeTags } =
      await import("./admin");
    ...
    await expect(backfillNormativeTags()).resolves.toMatchObject({
      createdCount: 1,
    });

    expect(apiClient.get).toHaveBeenCalledWith("/admin/catalog/options");
    expect(apiClient.post).toHaveBeenNthCalledWith(1, "/admin/tags", {
      name: "文档资料",
    });
  ...
  it("sends explicit first and second level category create payloads", async () => {
    ...
    expect(apiClient.post).toHaveBeenNthCalledWith(1, "/admin/categories", {
      name: "资料库",
      level: 1,
      sortOrder: 8,
    });
    expect(apiClient.post).toHaveBeenNthCalledWith(2, "/admin/categories", {
      name: "课程资料",
      level: 2,
      parentId: 1,
      sortOrder: 2,
