        ...
        <main className="main">
          <Routes>
            <Route
              path="/"
              element={
                <ContentPage openModal={openModal} setNotice={setNotice} />
              }
            />
            <Route
              path="/users"
              element={
                <UsersPage openModal={openModal} setNotice={setNotice} />
              }
            />
            <Route
              path="/reports"
              element={
                <ReportsPage openModal={openModal} setNotice={setNotice} />
              }
            />
            <Route
              path="/categories"
              element={
                <CategoriesPage openModal={openModal} setNotice={setNotice} />
              }
            />
            <Route
              path="/config"
              element={<ConfigPage setNotice={setNotice} />}
            />
...
function ContentPage({
  ...
  const [result, setResult] = useState<PageResult<AdminContentRow>>(() =>
    emptyPage<AdminContentRow>(),
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      setResult(
        await getAdminContentPage(activeTab, {
          page: String(page),
                  ...
                  {resourceRows.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>
                        ...
                        {statusIn(item, "COPYRIGHT_DOWN", "版权下架") && (
                          <span className="muted">版权锁定</span>
                        )}
                      </td>
                    </tr>
                  ))}
  ...
  return (
    <Panel title="用户账号管理">
      <div className="search-row">
        <label className="search-label" htmlFor="user-keyword">
          关键字
        </label>
        <input
          id="user-keyword"
        ...
        />
        <label className="search-label" htmlFor="user-status">
          状态
        </label>
        <select
          id="user-status"
      ...
      <p className="tip">
        说明：禁用后用户无法登录；恢复账号后，资源状态仍按资源管理页单独处理。
      </p>
    </Panel>
  );
}
...
function ReportsPage({
  ...
  const [page, setPage] = useState(1);
  const [result, setResult] = useState<PageResult<AdminComplianceRow>>(() =>
    emptyPage<AdminComplianceRow>(),
  );
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  ...
  const load = useCallback(async () => {
    ...
    try {
      setResult(
        await getAdminCompliancePage(activeTab, {
          page: String(page),
          size: String(ADMIN_PAGE_SIZE),
        }),
      );
      ...
      <section className="tab-content">
        {activeTab === "report" && (
          <Panel>
            ...
            />
            <p className="tip">
              举报处理可联动删除评论、下架资源、关闭帖子、删除回复或禁用用户。
            </p>
          </Panel>
        )}
...
function CategoriesPage({
  ...
  const [result, setResult] = useState<PageResult<AdminCategory>>(() =>
    emptyPage<AdminCategory>(),
  );
  const [options, setOptions] = useState<AdminCatalogOptions>(() =>
    emptyCatalogOptions(),
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [pageResult, nextOptions] = await Promise.all([
        getAdminCatalogPage(query.section, {
          ...query.params,
          page: String(page),
          size: String(ADMIN_PAGE_SIZE),
        }),
  ...
  return (
    <Panel title="分类标签管理">
      {loading ? (
        ...
        <>
          ...
          >
            {catalogFilterItems.map((item) => (
              <button
                key={item.key}
                className={
                  filter === item.key ? "catalog-tab active" : "catalog-tab"
                }
                          ...
                          onClick={() =>
                            openModal({
                              ...
                              onConfirm: async (name) => {
                                ...
                                await updateCategory(entityId(item), {
                                  name: nextName,
                                });
                                await reloadAfterAction();
                      ...
                      {statusIn(item, "ENABLED", "启用") ? (
                        <button
                          ...
                          onClick={() =>
                            void runCatalogAction(
                              isTag(item)
                                ? disableTag(entityId(item))
                                : disableCategory(entityId(item)),
                              `${item.id} 已禁用`,
                            )
                          }
                        >
                        ...
                        <button
                          ...
                          onClick={() =>
                            void runCatalogAction(
                              isTag(item)
                                ? updateTag(entityId(item), {
                                    status: "ENABLED",
                                  })
                                : updateCategory(entityId(item), {
                                    status: "ENABLED",
                                  }),
                              `${item.id} 已启用`,
                            )
          ...
          <div className="toolbar catalog-toolbar">
            <button
              className="btn add"
              onClick={() => setCatalogModal("first-category")}
            >
              新增一级分类
            </button>
            <button
              className="btn add"
              onClick={() => setCatalogModal("second-category")}
            >
              新增二级分类
            </button>
            <button className="btn add" onClick={() => setCatalogModal("tag")}>
              新增标签
            </button>
            <button className="btn edit" onClick={() => void runBackfill()}>
              一键补齐规范标签
            </button>
            <button
              className="btn edit"
      ...
      <p className="tip">
        说明：禁用后不可用于新资源发布；标签合并会迁移资源和求资源关联。
      </p>
    </Panel>
  );
}
  ...
  return (
    <div className="config-page">
      <Panel title="会员等级配置">
                ...
                <tr key={level.rawId || level.id || level.name}>
                  ...
                  <td>
                    <input
                      ...
                      value={level.min}
                      onChange={(event) =>
                        updateLevel(setConfig, index, "min", event.target.value)
                      }
                    />
                  </td>
                  <td>
                    <input
                      ...
                      value={level.max}
                      onChange={(event) =>
                        updateLevel(setConfig, index, "max", event.target.value)
                      }
                    />
                  </td>
                  <td>
        ...
        <button className="btn-save" onClick={saveLevels}>
          保存会员配置
        </button>
      </Panel>
      ...
      <Panel title="积分规则配置">
        <ConfigForm
          ...
          onChange={(updater) =>
            setConfig((current) => ({
              ...current,
              scoreRules: updater(current.scoreRules),
            }))
          }
          buttonText="保存积分规则"
          onSave={() =>
            void saveConfigItems(config.scoreRules, "积分规则已保存")
          }
        />
      </Panel>
      ...
      <Panel title="全局系统参数">
        <ConfigForm
          ...
          onChange={(updater) =>
            setConfig((current) => ({
              ...current,
              systemParams: updater(current.systemParams),
            }))
          }
          buttonText="保存系统参数"
          onSave={() =>
            void saveConfigItems(config.systemParams, "系统参数已保存")
          }
        />
      </Panel>
    </div>
...
function LogsPage() {
  ...
  const [page, setPage] = useState(1);
  const [result, setResult] = useState<PageResult<AdminLog>>(() =>
    emptyPage<AdminLog>(ADMIN_LOG_PAGE_SIZE),
  );
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
...
function renderReportActions(
  ...
  return (
    <>
      ...
      <button
        className={
          item.action === "delete-comment" ||
          item.action === "delete-reply" ||
          item.action === "disable-user"
            ? "btn del"
            : "btn off"
        }
        onClick={() =>
...
function renderComplaintActions(
  item: AdminComplaint,
  openModal: (modal: ModalState) => void,
  load: () => Promise<void>,
) {
...
function updateLevel(
  ...
  setConfig((config) => ({
    ...config,
    memberLevels: config.memberLevels.map((level, levelIndex) =>
  ...
  return (
    <div className="page-box" aria-label={ariaLabel}>
      <div className="page-summary">
        共 {total} 条 / 第 {currentPage} 页 / 共 {pageCount} 页
      </div>
...
function CatalogEntityModal({
  ...
  const enabledParents = options.firstLevelCategories.filter(
  ...
  const canSubmit =
    mode === "tag"
      ? Boolean(selectedTag)
      : Boolean(name.trim()) &&
        (mode === "first-category" || Boolean(parentId));
  ...
  return (
    <div className="modal" role="dialog" aria-modal="true" aria-label={title}>
      <div className="modal-box catalog-modal">
        ...
        {mode === "tag" && candidates.length === 0 && (
          <div className="modal-help">
            当前规范词均已存在标签，可使用一键补齐刷新状态。
          </div>
        )}
        {mode === "second-category" && enabledParents.length === 0 && (
          <div className="modal-help">
            暂无启用的一级分类，请先新增或启用一级分类。
          </div>
        )}
        {error && <div className="modal-error">{error}</div>}
        <div className="modal-actions">
...
function ErrorState({
  ...
  return (
    <div className="state-box error-state">
      <span>{message}</span>
      <button className="btn-search" onClick={onRetry}>
        重试
      </button>
    </div>
  );
...
function catalogQuery(filter: CatalogFilter): {
  section: AdminCatalogSection;
  params: Record<string, string>;
} {
  if (filter === "level1")
    return { section: "category", params: { level: "1" } };
  if (filter === "level2")
    return { section: "category", params: { level: "2" } };
...
function availableTagCandidates(
  options: AdminCatalogOptions,
): AdminCatalogTagCandidate[] {
...
function statusIn(
  item: { status?: string; rawStatus?: string },
  ...statuses: string[]
) {
  return (
    statuses.includes(item.rawStatus || "") ||
...
function statusClass(status: string) {
  if (["正常", "启用", "已发布", "已通过", "已处理"].includes(status))
    return "ok";
  if (["待审核", "待处理", "处理中", "风险复核"].includes(status))
    return "pending";
  if (
    ["已禁用", "已下架", "版权下架", "已删除", "已驳回", "已关闭"].includes(
      status,
    )
  )
    return "danger";
