...
- Primary readable reference documents:
  `D:\图片\详细设计说明书.md`,
  `D:\图片\资源分享论坛——UML建模文档.md`, and
  `D:\图片\资源分享论坛——数据库设计说明书.md`.
- Cross-check references: detailed design, UML modeling, coding standard, and database design PDFs.
...
| Spec deployment acceptance is mapped to evidence | The three Markdown documents, `规范.docx`, and the four PDFs are mapped to runtime, API, database, security, logging, backup, and frontend handoff evidence without changing public contracts. | `SPEC_DEPLOYMENT_ACCEPTANCE.md`; `BackendDeploymentConfigTest` |
